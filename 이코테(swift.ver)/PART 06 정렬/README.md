...
## 위에서 아래로
크기에 상관없이 나열되어 있는 수를 큰수부터 작은 수의 순서로 정렬해야 한다.   
내림차순으로 정렬하는 프로그램을 만드시오.  
   
위의 문제와 같은 경우가 기본적인 정렬을 할 수 있는지 물어보는 문제이다.   
수가 1이상 100,000이하이므로 어떠한 정렬 알고리즘을 사용해도 문제를 해결할 수 있지만   
가장 간결해지는 정렬 라이브러리를 사용하는 것이 효과적이다.   
   
## 성적이 낮은 순서로 출력하기
N명의 학생 정보가 있다. 학생 정보는 학생의 이름과 학생의 성적으로 구분된다.   
각 학생의 이름과 성적 정보가 주어졌을 때 성적이 낮은 순서대로 학생의 이름을 출력하는 프로그램을 작성하시오.   
   
해당 문제는 최대 100,000 개까지 데이터 정보가 입력될 수 있으므로 최악의 경우 O(NlogN)을 보장하는 알고리즘을 이용하거나   
O(N)을 보장하는 계수 정렬을 이용하면 된다.   
   
## 두 배열의 원소 교체
두개의 배열 A와 B를 가지고 있다. 두 배열은 N개의 원소로 구성되어 있으며, 모두 자연수이다.   
최대 K번 바꿔치기를 수행할 수 있는데, A에 있는 원소 하나와 B에 있는 원소 하나를 골라서 바꾸는 것이다.   
최종 목표는 A의 모든 원소의 합이 최대가 되도록 하는것이다.   
   
해당 문제의 가장 포인트는 매번 배열 A의 가장 작은 원소를 골라서 배열 B의 가장 큰 원소와 바꾸는 것이다.   
A를 오름차순으로 정렬하고, B를 내림차순으로 정렬한다.   
그리고 두 배열의 원소의 첫번째 인덱스부터 차례대로 비교하며 A의 원소가 B의 원소보다 작을떄에만 교체를 수행한다.   
최대 100,000개까지 입력할 수 있으므로 O(NlogN)을 보장하는 정렬 알고리즘을 이용해야한다.   
